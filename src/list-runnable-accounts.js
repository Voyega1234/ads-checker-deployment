...
async function main() {
  ...
  const runnable = results.filter((entry) => entry?.count > 0);
  const totalCreatives = runnable.reduce((sum, entry) => sum + entry.count, 0);

  console.log(
    JSON.stringify(
      {
        accountsAccessible: accounts.length,
        accountsWithAds: runnable.length,
        totalCreatives,
        placementsIfThree: totalCreatives * 3,
        averageCreativesPerRunnableAccount: runnable.length
          ? Number((totalCreatives / runnable.length).toFixed(2))
          : 0,
        accounts: runnable
      },
      null,
